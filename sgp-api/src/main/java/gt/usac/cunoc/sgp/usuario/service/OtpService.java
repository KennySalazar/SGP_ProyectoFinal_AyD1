...
public class OtpService {
    ...
    public OtpService(OtpChallengeRepository challenges, PasswordEncoder passwordEncoder, EmailService emailService, OtpProperties properties, Clock clock) {
    ...
    public OtpChallenge issue(UserAccount user, OtpPurpose purpose) {
        ...
        OtpChallenge latest = challenges.findTopByUserAndPurposeAndConsumedAtIsNullOrderByCreatedAtDesc(user, purpose).orElse(null);
        if (latest != null && !latest.isExpired(now) && latest.getCreatedAt().plusSeconds(properties.getResendCooldownSeconds()).isAfter(now)) {
            throw new OtpRateLimitException();
        }
        challenges.invalidateActive(user, purpose, now);
        String code = generateCode();
        OtpChallenge challenge = challenges.save(new OtpChallenge(user, purpose, passwordEncoder.encode(code), now.plusSeconds(properties.getExpirationMinutes() * 60L), properties.getMaxAttempts(), now));
    ...
    public UserAccount verify(UUID challengeId, UserAccount expectedUser, OtpPurpose purpose, String code) {
        ...
        if (challenge == null || challenge.getPurpose() != purpose || !challenge.getUser().getId().equals(expectedUser.getId())) throw invalidOtp();
    ...
    private void verifyCode(OtpChallenge challenge, String code) {
        Instant now = clock.instant();
        if (code == null || challenge.isConsumed() || challenge.isExpired(now) || !challenge.hasAttemptsRemaining()) throw invalidOtp();
    ...
    private ApiException invalidOtp() {
        return new ApiException(HttpStatus.BAD_REQUEST, "invalid_otp", "Codigo OTP invalido", "Codigo OTP invalido o expirado");
    }
}
