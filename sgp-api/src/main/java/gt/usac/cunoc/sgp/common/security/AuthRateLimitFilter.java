...
public class AuthRateLimitFilter extends OncePerRequestFilter {
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) {
        ...
        return !(uri.equals("/api/v1/auth/login") || uri.equals("/api/v1/auth/register")
            || uri.equals("/api/v1/auth/password-recovery") || uri.equals("/api/v1/auth/refresh"));
    }
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        WindowCounter counter = counters.compute(key, (ignored, existing) -> {
            if (existing == null || existing.startedAt.plusSeconds(WINDOW_SECONDS).isBefore(now)) return new WindowCounter(now, 1);
            return new WindowCounter(existing.startedAt, existing.count + 1);
        });
        if (counter.count > MAX_REQUESTS) {
            ProblemDetail problem = ProblemDetails.create(429, "Demasiadas solicitudes", "Espere antes de volver a intentarlo", "rate_limited", request);
