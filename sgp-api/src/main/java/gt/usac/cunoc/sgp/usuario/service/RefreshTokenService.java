...
public class RefreshTokenService {
  ...
  public String issue(UserAccount user) {
    ...
    tokens.save(
        new RefreshToken(
            user, TokenHasher.sha256(raw), now.plusMillis(jwtService.refreshExpirationMs()), now));
    return raw;
  }
  ...
  public RotatedRefreshToken rotate(String raw) {
    ...
    RefreshToken stored =
        tokens
            .findByTokenHashWithUser(TokenHasher.sha256(raw))
  ...
  private ApiException invalidRefreshToken() {
    return new ApiException(
        ...
        "Sesion no valida",
        "El token de renovacion es invalido o expiro");
  }
