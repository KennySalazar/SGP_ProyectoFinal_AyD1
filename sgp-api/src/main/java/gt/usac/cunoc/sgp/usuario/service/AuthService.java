...
public class AuthService {
  ...
  public ChallengeResponse register(RegisterRequest request) {
    ...
    Role role =
        roles
            .findByName(RoleName.ESTUDIANTE)
            .orElseThrow(() -> new IllegalStateException("Falta el rol ESTUDIANTE"));
    UserAccount user =
        users.save(
            new UserAccount(email, passwordEncoder.encode(request.password()), role, false, false));
  ...
  public ChallengeResponse resendRegistration(String rawEmail) {
    ...
    var challenge = otpService.issue(user, OtpPurpose.REGISTRO);
    return new ChallengeResponse(
        challenge.getId(), challenge.getExpiresAt(), "Se envio un nuevo codigo de verificacion");
  }
  ...
  public MessageResponse verifyRegistration(String rawEmail, UUID challengeId, String code) {
    ...
    return new MessageResponse(
        "Correo verificado. La cuenta queda pendiente de activacion por un Catedratico.");
  }
  ...
  public RefreshFlowResult refresh(String rawRefreshToken) {
    ...
    return new RefreshFlowResult(
        new AccessTokenResponse(access, "Bearer", jwtService.accessExpirationMs()),
        rotated.rawToken());
  }
  ...
  public ChallengeResponse requestTwoFactorChange(
      String rawEmail, String currentPassword, OtpPurpose purpose) {
  ...
  public MessageResponse confirmTwoFactorChange(
    ...
    user.setTwoFactorEnabled(purpose == OtpPurpose.ACTIVAR_2FA);
  ...
  private LoginFlowResult issueLogin(UserAccount user) {
    ...
    return new LoginFlowResult(
        LoginResponse.token(access, jwtService.accessExpirationMs()), refresh);
  }
  ...
  private ApiException invalidCredentials(String detail) {
    return new ApiException(
        HttpStatus.UNAUTHORIZED, "invalid_credentials", "Credenciales invalidas", detail);
  }
