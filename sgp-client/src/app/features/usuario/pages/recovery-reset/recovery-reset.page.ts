...
export class RecoveryResetPage {
  ...
  readonly form = this.fb.nonNullable.group({
    ...
    newPassword: ['', [Validators.required, Validators.minLength(10), Validators.pattern(/^(?=.*[A-Za-z])(?=.*\d).+$/)]]
  });

  submit(): void {
    if (this.form.invalid) return;
    this.http.post<MessageResponse>('/api/v1/auth/password-recovery/verify', this.form.getRawValue()).subscribe(() => this.done.set(true));
  }
}
